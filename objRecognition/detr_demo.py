...
def get_object(im):
    scores, boxes = detect(im)
    if len(scores) == 0:
        return {}
    answers = []
    for i in range(len(scores)):
        answer = {}
        answer['class'] = CLASSES[torch.argmax(scores[i])]
        answer['x'] = boxes[i][0].item()
        answer['y'] = boxes[i][1].item()
        answer['width'] = boxes[i][2].item() - boxes[i][0].item()
        answer['height'] = boxes[0][3].item() - boxes[i][1].item()
        answer['probability'] = torch.max(scores[i]).item()
